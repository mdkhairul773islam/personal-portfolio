...
function createVisualization() {
    ...
    const itemHeight = 300; // Height between cards (vertical spacing)
    const cardHeight = 200; // Actual card height
    const cardWidth = 340; // Card width for vertical layout
    const margin = { top: 60, right: 60, bottom: 60, left: 60 }; // Balanced margins
    ...
    // Create timeline scale for Y-axis (vertical layout)
    const yScale = d3.scaleLinear()
        .domain([2011, 2025])
        .range([margin.top, height - margin.bottom]);

    // Draw main timeline axis (vertical line in center)
    ...
    // Year markers with horizontal lines and comet animation
    const years = d3.range(2011, 2026, 2);
    svg.selectAll(".year-marker")
        ...
        .each(function(d, i) {
            ...
            // Comet animation - moving left and right on the year mark line
            function createComet() {
                const tickLeft = axisX - 8;
                const tickRight = axisX + 8;
                const tickWidth = tickRight - tickLeft;
                
                // Alternating direction
                const direction = Math.random() > 0.5 ? 'right' : 'left';
                const startX = direction === 'right' ? tickLeft : tickRight;
                const endX = direction === 'right' ? tickRight : tickLeft;
                ...
                const trail = g.append("line")
                    ...
                    .attr("y1", yearY)
                    .attr("y2", yearY)
                    .attr("stroke", direction === 'right' ? "url(#comet-gradient-right)" : "url(#comet-gradient-left)")
                    .attr("stroke-width", 2)
                    .attr("stroke-linecap", "round")
                    .attr("opacity", 0);
                ...
                trail.transition()
                    ...
                    .attrTween("x1", function() {
                        return function(t) {
                            const currentX = startX + (tickWidth * t * (direction === 'right' ? 1 : -1));
                            return direction === 'right' 
                                ? Math.max(tickLeft, currentX - 8)
                                : Math.min(tickRight, currentX + 8);
                        };
                    })
                    .attrTween("x2", function() {
                        return function(t) {
                            return startX + (tickWidth * t * (direction === 'right' ? 1 : -1));
                        };
                    })
                    .transition()
                    .duration(100)
    ...
    // Create gradients for comet trail (right direction)
    const cometGradientRight = svg.append("defs")
        .append("linearGradient")
        .attr("id", "comet-gradient-right")
        .attr("x1", "0%")
        .attr("y1", "0%")
        .attr("x2", "100%")
        .attr("y2", "0%");
    
    cometGradientRight.append("stop")
        .attr("offset", "0%")
    ...
    // Create gradients for comet trail (left direction)
    const cometGradientLeft = svg.append("defs")
        .append("linearGradient")
        .attr("id", "comet-gradient-left")
        .attr("x1", "100%")
        .attr("y1", "0%")
        .attr("x2", "0%")
        .attr("y2", "0%");
    
    cometGradientLeft.append("stop")
        .attr("offset", "0%")
        .attr("stop-color", "#8B7355")
        .attr("stop-opacity", 0);
    ...
    experiences.forEach((exp, i) => {
        const cardY = i * itemHeight + margin.top;
        const isLeft = i % 2 === 0;
        ...
        // Calculate timeline bar position (vertical bar on center line)
        const barHeight = yScale(exp.endYear) - yScale(exp.startYear);
        
        // Bar background
        group.append("rect")
            .attr("x", axisX - 3)
            .attr("y", yScale(exp.endYear))
            .attr("width", 0)
        ...
        const boxX = isLeft ? (axisX - boxWidth - 30) : (axisX + 30);
        const boxY = cardY;
        ...
        const connectorStartX = boxX + (isLeft ? boxWidth : 0);
        const connectorStartY = boxY + boxHeight / 2;
        const connectorEndX = axisX;
        const connectorEndY = yScale((exp.startYear + exp.endYear) / 2);
        ...
        const mainSpiral = group.append("path")
            .attr("d", `M ${connectorStartX} ${connectorStartY}
                       C ${connectorStartX - 10} ${connectorStartY + spiralStrength * direction * 1.2},
                         ${midX - spiralStrength * 0.7} ${midY + spiralStrength * direction * 2},
